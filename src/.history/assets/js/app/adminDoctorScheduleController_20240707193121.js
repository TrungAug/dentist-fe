    ...
    $scope.processChangeDoctorSchedule = (selectedDoctors, originalShifts, selectedShifts, selectedDates, isChangeDate) => {
        ...
        const findShifts = () => {
            if (originalShiftsLenght > selectedShiftsLenght) {
                deleteShifts = originalShifts.filter(item => !selectedShiftsString.has(item.split('-')[1].toString()));
                updateShifts = originalShifts.filter(item => selectedShiftsString.has(item.split('-')[1].toString()));
            } else if (originalShiftsLenght < selectedShiftsLenght) {
                addShifts = selectedShiftsString.filter(item => !originalShiftsSplit.includes(item));
                updateShifts = originalShifts.filter(item => selectedShiftsString.has(item.split('-')[1].toString()));
            }
        ...
        if (changeDate) {
            ...
            if (originalShiftsLenght > selectedShiftsLenght) {
                ...
                var dataArrayDelete = $scope.generateScheduleArray(selectedDoctors, dates, deleteShifts)
                console.log("dataArrayDelete", dataArrayDelete);
                ...
                var dataArrayUpdate = $scope.generateScheduleArray(selectedDoctors, dates, updateShifts)
                console.log("dataArrayUpdate", dataArrayUpdate);

                dataArrayDelete.forEach(dataDel => {
                    //var dataDelJSON=angular.toJson(dataDel)
                    // $http.delete(url + '/soft-delete-doctor-schedule/' + dataDel.doctorScheduleId).then(response => {
                    //     dataArrayUpdate.forEach(dataUpdate => {
                    //         var dataUpdateJSON = angular.toJson(dataUpdate)
                    //         $http.put(url + '/doctor-schedule/' + dataUpdate.doctorScheduleId, dataUpdateJSON).then(resp => {
                    //             Swal.fire({
                    //                 title: "Thành công!",
                    ...
                    //             })
                    //         }).catch(err => {
                    //             console.log("Có lỗi khi thay đổi lịch làm việc ", err);
                    //         })
                    //     })
                    // }).catch(error => {
                    //     console.log("Có lỗi khi xóa lịch làm việc ", error);
                    // })
                })
                ...
                var dataArrayAdd = $scope.generateScheduleArray(selectedDoctors, dates, addShifts)
                console.log("dataArrayAdd", dataArrayAdd);
                // tìm ca còn lại cập nhật ngày  
                updateShifts = originalShifts.filter(item => {
                    var id = item.split('-')[1]
                    return selectedShiftsString.includes(id)
