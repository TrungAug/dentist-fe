...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.initDataRequest = (originalAppointment, originalAPR) => {
    ...
    $scope.saveAppointment = async () => {
        $scope.initDataRequest($scope.originalAppointment, $scope.originalAPR)
    ...
    $scope.updateAppointment = () => {
        $scope.initDataRequest($scope.originalAppointment, $scope.originalAPR)
        // console.log("form app",$scope.formApp);
        ...
        const loadDoctorUnavailability = () => {
            ...
            $scope.processDoctorUnavailability().then(dataDu => {
                eventArrDu = dataDu.map(du => ({
                    ...
                    color: $scope.getColorForStatusId(du.appointment.appointmentStatus.status)
                }));
                mergeEvents();
            });

            $scope.getAllDoctorScheduleExceptDeleted().then(dataDs => {
                eventArrDs = dataDs.map(ds => ({
                    id: "",
            ...
            const mergeEvents = () => {
                if (eventArrDu.length > 0 && eventArrDs.length > 0) {
                    eventArrDs.forEach(ds => {
                        let matchingEvent = eventArrDu.find(du =>
                        (moment(du.start).isBetween(ds.start, ds.end, undefined, '[]') ||
                            moment(du.end).isBetween(ds.start, ds.end, undefined, '[]'))
                        );
                        if (matchingEvent) {
                            ds.color = matchingEvent.color;
                        }
                    });
                }
                updateCalendar();
            ...
            //        // ;
            //         dataDu.forEach(du => {
            //             let color = $scope.getColorForStatusId(du.appointment.appointmentStatus.status);
            //             let event = {
            //                 id: du.appointment.appointmentId,
            //                 start: du.appointment.appointmentDate + "T" + du.timeOfShift.beginTime,
            //                 end: du.appointment.appointmentDate + "T" + du.timeOfShift.endTime,
            ...
            const renderCalendar = () => {
                ...
                calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    //     $scope.getDoctorScheduleShiftsExcludingDeleted($scope.fullCalendarDate, arg.resource.id).then(result => {
                    //         var tdElements = calendarEl.querySelectorAll('td.fc-widget-content.fc-minor, td.fc-widget-content.fc-major')
                    //         result.forEach(item => {
                    //             var shift = item[0]
                    //             tdElements.forEach(td => {
                    //                 var tdDate = new Date(td.getAttribute('data-date'))
                    //                 var tdTime = moment(tdDate).format("HH:mm:ss")
                    //                 var beginTime = moment(shift.beginTime, "HH:mm:ss").subtract(30, 'minutes').format("HH:mm:ss");
                    //                 var endTime = moment(shift.endTime, "HH:mm:ss").subtract(30, 'minutes').format("HH:mm:ss");

                    //                 // console.log("tdTime", tdTime);
                    //                 // console.log("beginTime của " + arg.resource.id, beginTime);
                    //                 // console.log("endTime của " + arg.resource.id, endTime);
                    //                 if (moment(tdTime, "HH:mm:ss").isBetween(moment(beginTime, "HH:mm:ss"), moment(endTime, "HH:mm:ss"))) {
                    //                     td.classList.add('fc-custom-available');
                    //                 }else{
                    //                     td.classList.add('fc-custom-past');
                    //                 }
                    //             })
