app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.isCheduled = (doctorId) => {   
        let doctorIds=[]
        console.log("$scope.listDoctorScheduleByTimeRangesDB",$scope.listDoctorScheduleByTimeRangesDB);
        for(let key in $scope.listDoctorScheduleByTimeRangesDB){
            doctorIds.push(parseInt(key))
        }
        console.log("doctorIds doctorIds",doctorIds);
        // return $scope.listDoctorScheduleByTimeRangesDB.some(obj => {
        //     return obj === doctorId
        // })
    }

    $scope.detailByShift=(startDate,endDate,shiftId,listDoctorScheduleByTimeRangesDB)=>{
       // $scope.getDoctorScheduleByTimeRange(arg.startStr, arg.endStr, null)
        console.log("startDate",startDate);
        console.log("endDate",endDate);
        console.log("shiftId",shiftId);
        console.log("listDoctorScheduleByTimeRangesDB",listDoctorScheduleByTimeRangesDB);
    }
    ...
    $scope.getDoctorScheduleByTimeRange = (startStr, endStr, paramsFiler) => {
        ...
        if (!paramsFiler) {
            ...
            $http.get(url + '/map-ds-by-time-range', { params: params }).then(response => {
                // click chọn thời gian trên lịch
               $scope.listDoctorScheduleByTimeRangesDB = response.data
               console.log("$scope.listDoctorScheduleByTimeRangesDB dm",$scope.listDoctorScheduleByTimeRangesDB);
            })
        } else {
            params = {
    ...
    $scope.countShiftByTimeRange = (listDSByTimeRangesDB, shift) => {
        let filterData = listDSByTimeRangesDB.filter(ds => ds.shift.shiftId === shift.shiftId)
        let dsUniqueDoctors = []
        let uniqueDoctorIds = []
    ...
    $scope.getHourWorking=(listDSByTimeRangesDB,doctorId) => {          
        ...
        let filterData = listDSByTimeRangesDB.filter(ds => ds.doctor.doctorId === doctorId)
