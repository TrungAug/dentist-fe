...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.initializeCalendarAppointment = () => {
        $scope.processDoctorsWithAppointmentStatus().then(result => {
            ...
            for (var key in result) {
                if (result.hasOwnProperty(key)) {
                    ...
                    result[key].forEach(at => {
                        var child = {
                            id: key + "-" + at.appointment_StatusId,
                            title: at.status,
                        }
                        childArr.push(child)
                    })
            ...
            $scope.processDoctorUnavailability().then(du => {
                console.log("du", du);
                var eventArr = []
                // du.forEach(appoinment => {
                //     var event = {
                //         id: '3',
                //         title: 'Cuộc hẹn hủy',
                //         start: '2024-07-10T08:00:00',
                //         end: '2024-07-10T09:00:00',
                //         resourceId: appoinment.doctor.doctorId+"-"+appoinment.appointmentStatus.appointment_StatusId,
                //         color: 'red'
                //     }
                // })
                ...
                var calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    //resources:'http://localhost:8081/api/v1/auth/doctor',
                    resources: resources,
                    selectable: true,
                    selectMirror: true,
                    select: function (arg) {

                        console.log("arg selected", arg);
                    },
