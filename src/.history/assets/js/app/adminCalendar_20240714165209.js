...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getListAppointment = () => {
        $http.get(url + '/appointment').then(resp => {
            ...
            let pastLength=$scope.listAppointmentDBToday.length
            console.log("todayLength",todayLength);
            let a=((todayLength - pastLength) / (pastLength==0?todayLength:pastLength)) * 100
            console.log("a",a);
            $scope.appFluctuate = Math.round();
