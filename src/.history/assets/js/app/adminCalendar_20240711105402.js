...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.isShiftAvailableCalendar = (tdDate,resourceId) => {
        // console.log("resourceId",resourceId);
        // console.log("$scope.listDuDB",$scope.listDuDB);
        var time = moment(tdDate.getTime()).format("HH:mm:ss")
        var checkData = $scope.listDuDB.filter(du => du.timeOfShift.beginTime === time&&du.appointment.doctor.doctorId)
        return checkData.length > 0 // bận lich
    }
                ...
                var calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    datesRender: function (info) {
                        ...
                        trElements.forEach(tr => {
                            var resourceId = tr.getAttribute('data-resource-id');
                           // console.log("Resource ID:", resourceId)
                            tdElements.forEach(td => {
                                ...
                                //     td.remove()
                                // }
                                var isDu = $scope.isShiftAvailableCalendar(tdDate,resourceId)
