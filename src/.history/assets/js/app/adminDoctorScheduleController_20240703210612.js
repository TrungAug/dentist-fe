    ...
    $scope.crudDoctorSchedule = () => {
        ...
        $scope.isExistDoctorSchedule = (date, shiftId, scheduleOrigin) => {
            return new Promise((resolve, reject) => {
                ...
                $scope.processDoctorScheduleByDoctor(doctorId).then(result => {
                    console.log("result lịch", result)
                    console.log("date", convertedDate)
                    filteredResults = result.filter(ds => {
                        var dsDateWithoutMillis = removeMilliseconds(ds.date);
                        var convertedDateWithoutMillis = removeMilliseconds(convertedDate);

                        return dsDateWithoutMillis === convertedDateWithoutMillis
                    })
                    // if (filteredResults) {
                    //     filteredResults = filteredResults.filter(ds => ds.timeOfShift.shift.shiftId === shiftId)
                    // }
                    var isExistDoctorSchedule = filteredResults && filteredResults.length > 0
                    console.log("isExistDoctorSchedule", isExistDoctorSchedule);
        ...
        $scope.changeDoctorSchedule = async () => {
            ...
            await $scope.isExistDoctorSchedule(date, shiftId, scheduleOrigin)
