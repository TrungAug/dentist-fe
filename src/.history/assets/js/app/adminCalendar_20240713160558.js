...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    // $scope.isShowServiceUp = () => {
    //     return $scope.listServiceByDentalIssuesDBUp.length > 0
    // }
    ...
    $scope.updateSelectedServices = (service, isUpdate) => {
        // if (service.checked) {
        //     if(isUpdate){
        //         $scope.selectedServicesUp.push(service);
        //     }
        //     $scope.selectedServices.push(service);
        // } else {
        //     if(isUpdate){
        //         const index = $scope.selectedServicesUp.findIndex(s => s.serviceId === service.serviceId);
        //     if (index !== -1) {
        //         $scope.selectedServicesUp.splice(index, 1)
        //     }
        //     }
    ...
    $scope.toggleAll = (isUpdate) => {
        // if (isUpdate) {
        //     const selectAll = document.getElementById('selectAllUp').checked;
        //     $scope.selectedServicesUp = []
        //     $scope.listServiceByDentalIssuesDBUp.forEach(service => {
    ...
    //         }
    //     })
    //     console.log("$scope.selectedServices Up", $scope.selectedServices)
    // }



    // $scope.updateSelectedServicesUp = (service) => {
    //     if (service.checked) {
    //         $scope.selectedServices.push(service);
    //     } else {
    //         const index = $scope.selectedServices.findIndex(s => s.serviceId === service.serviceId);
    ...
    // $scope.toggleCheckboxUp = function ($event, service) {
    //     $event.stopPropagation();
    //     service.checked = !service.checked;
    //     $scope.updateSelectedServicesUp(service);
    // }
    ...
    $scope.onChangeTimeOfShift = (tos, isUpdate) => {
        if (tos.checked) {
            if (isUpdate) {
    ...
    $scope.processChangeAppStatus = (statusId) => {
        var st = $scope.listAppointmentStatusBD.find(s => s.appointment_StatusId === statusId)
        if (!st) {
    ...
    $scope.updateAppointment = () => {
        ...
        //Cập nhật hoàn thành nếu dataArrayServiceReq==[] cảnh báo, hủy, không đến, hoãn, => cập nhật dịch vụ
        //$scope.processDataUpdate($scope.originalAS, dataArrayServiceReq)
        //Cập 
        ...
        const handleApiRequest = (deleteUrl, postUrl, putUrl, data, idKey) => {
            ...
            if (data.deleteArr.length !== 0) {
                data.deleteArr.forEach(item => {
                    promises.push($http.delete(`${deleteUrl}/${item[idKey]}`))

                });
            }

            if (data.postArr.length !== 0) {
                data.postArr.forEach(item => {
                    promises.push($http.post(postUrl, item))
                });
            }

            if (data.updateArr.length !== 0) {
                data.updateArr.forEach(([original, updated]) => {
                    original.forEach((oItem, index) => {
                        const id = oItem[idKey];
                        const uItem = updated[index]
                        promises.push($http.put(`${putUrl}/${id}`, uItem))
                    })
                });
            }
        ...
        Promise.all([
            handleApiRequest("/soft-delete-appointment-patient-record", "/appointment-patient-record", "/appointment-patient-record", reqAprData, "appointmentPatientRecordId"),
            handleApiRequest("/soft-delete-appointment", "/appointment", "/appointment", reqAppData, "appointmentId"),
            handleApiRequest("/soft-delete-doctor-unavailability", "/doctor-unavailability", "/doctor-unavailability", reqDuData, "doctorUnavailabilityId"),
            handleApiRequest("/soft-delete-appointment-service", "/appointment-service", "/appointment-service", reqAsData, "appointment_ServiceId")
        ]).then(result => {
            Swal.fire({
                title: "Thành công!",
        ...
        // if(reqAprData.deleteArr.length!=0){
        //     reqAprData.deleteArr.forEach(apr=>{
        //         $http.deleted(url+"/soft-delete-appointment-patient-record/"+apr.appointmentPatientRecordId)
        //     })
        // }

        // if(reqAprData.postArr.length!=0){
        //     reqAprData.postArr.forEach(aprReq=>{
        //         $http.post(url+"/appointment-patient-record",aprReq)
        //     })
        // }

        // if(reqAprData.updateArr.length!=0){
        //     reqAprData.updateArr.forEach(arr=>{
        //         let id=""
        //         arr[0].forEach(oApr=>{
        //             var id=oApr.appointmentPatientRecordId
        //             arr[1].forEach(aprReq=>{
        //                 $http.put(url+"/appointment-patient-record/"+id,aprReq)
        //             })
        //         })
        //     })
        // }
        ...
        // console.log("reqAprData",reqAprData.deleteArr);
        // console.log("reqAprData",reqAprData.postArr);
        // console.log("reqAprData",reqAprData.updateArr);

        // console.log("$scope.selectedServicesUp", $scope.selectedServicesUp);
        // console.log("$scope.isValidServiceUp", $scope.isValidServiceUp);
        console.log("reqAprData", reqAprData);
        console.log("reqAppData", reqAppData);
        console.log("reqDuData", reqDuData);
        console.log("reqAsData", reqAsData);


        // $scope.originalAS.length > dataArrayServiceReq => update và xóa aps
        // $scope.originalAS.length < dataArrayServiceReq => update và thêm aps
        // $scope.originalAS.length = dataArrayServiceReq => update
        // áp dụng logic trên cho $scope.originalDU,dataArrayDUReq
    ...
    $scope.initializeUIComponentsModal = () => {
        ...
        $('.drgpicker-reExam, .drgpicker-up').daterangepicker(
            {
        ...
        $('#appointmentStatusUp').on('change', function () {
            $timeout(function () {
                var selectedVal = $('#appointmentStatusUp').val()
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                eventClick: function (arg) {
                    ...
                    $scope.formUpApp = {
                        ...
                        appointmentDate: moment(originalApp.appointmentDate, ('YYYY-MM-DD')).format('DD/MM/YYYY'),
