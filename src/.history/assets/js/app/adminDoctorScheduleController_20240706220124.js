app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getDoctorShiftsExcludingDeleted = (date, doctorId) => {
        ...
        $http.get(url + '/get-doctor-shifts-excluding-deleted', { params: params }).then(response => {
            ...
            response.data.forEach(item => {
                ...
                $scope.originalIds.push(item[1]+'-'+item[0].shiftId)
            });
        })
    }
    ...
    $scope.changeDoctorSchedule = () => {
        //các id cũ
        console.log("$scope.originalIds changeDoctorSchedule: ", $scope.originalIds);
        var originalIdsArray = $scope.originalIds.split('-');
        var originalDoctorScheduleIds= originalIdsArray[0]
        var originalDoctorShiftIds=originalIdsArray[1]
        // $scope.originalDoctorScheduleIds = []
        // $scope.originalDoctorShiftIds = []

        console.log("originalDoctorScheduleIds",originalDoctorScheduleIds);
        console.log("originalDoctorShiftIds",originalDoctorShiftIds);
