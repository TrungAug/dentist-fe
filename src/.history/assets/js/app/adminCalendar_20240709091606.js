...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.isShiftAvailable = (timeOfShiftId) => {
        ...
        //var checkData =$scope.listTOSFilter.filter(tos=>tos[1].)
    }
            ...
            $scope.getTimeOfShiftAvailable(shiftId, dateSelected, doctorId)
                .then(function (data) {
                    if ($scope.comparasionDate(dateSelected)) {
                        ...
                        $scope.listTOSFilter = data.filter((tos) => {
    ...
    $scope.addMinutesToTime = (time, minutesToAdd) => {
        // Tách chuỗi thời gian thành giờ, phút, giây
        const [hours, minutes, seconds] = time.split(':').map(Number);

        // Tạo một đối tượng Date với các thông số giờ, phút, giây
        const date = new Date();
        date.setHours(hours);
        ...
        // Định dạng lại thời gian sau khi cộng
        const newHours = String(date.getHours()).padStart(2, '0');
        const newMinutes = String(date.getMinutes()).padStart(2, '0');
        const newSeconds = String(date.getSeconds()).padStart(2, '0');

        // Trả về thời gian mới dưới dạng "HH:mm:ss"
        return `${newHours}:${newMinutes}:${newSeconds}`;
    }

    $scope.onChangeTimeOfShiftId = (timeOfShiftId) => {
        var totalTime = $scope.getTotalTime()
        var arrTos=$scope.listTOS.filter(arrt=>arrt[1].timeOfShiftId==timeOfShiftId)
        var tos=arrTos[0][1]
        var time=arrTos[0][1].beginTime
        var validTime=$scope.addMinutesToTime(time,totalTime)
    ...
    $scope.initializeUIComponents = () => {
        ...
        $('#dental-issues').on('change', function () {
            $timeout(function () {
                ...
                $scope.getServiceByDentalIssues(selecteShiftdValues)
                console.log("selecteShiftdValues", selecteShiftdValues);
            });
        });
    }
