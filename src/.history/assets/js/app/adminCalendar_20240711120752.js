...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.processDSWithAppointmentStatus = (calendarDate) => {
        var params = {
            date: calendarDate
        }
        return new Promise((resolve, reject) => {
    ...
    $scope.initializeCalendarAppointment = () => {
        var fullCalendarDate = new Date();

        function initializeFullCalendar(fullCalendarDate, resources, dataDu) {
            ...
            var calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                events: generateEventArray(dataDu),
                resources: resources,
                selectable: true,
                selectMirror: true,
                select: function (arg) {
                ...
                datesRender: function (info) {
                    fullCalendarDate = info.view.currentStart;
                    var now = new Date();
                    ...
                    // -----
                    $scope.processDSWithAppointmentStatus(fullCalendarDate).then(result => {
                        var newResources = [];
                        for (var key in result) {
                            if (result.hasOwnProperty(key)) {
                                ...
                                newResources.push(obj);
                            }
                        }
                        newResources.sort((a, b) => a.id - b.id);
        ...
        function generateEventArray(doctorUnavailabilityData) {
            var eventArr = [];
            doctorUnavailabilityData.forEach(du => {
        ...
        $scope.processDSWithAppointmentStatus(fullCalendarDate).then(result => {
            ...
            $scope.processDoctorUnavailability().then(dataDu => {
                initializeFullCalendar(fullCalendarDate, resources, dataDu)
            })
        })
    ...
    //                     fullCalendarDate = info.view.currentStart;
    //                     console.log("FullCalendar Date:", fullCalendarDate);
    //                     var now = new Date();
    //                     now.setMinutes(now.getMinutes() - 15)
    //                     var tdElements = calendarEl.querySelectorAll('td.fc-widget-content.fc-minor, td.fc-widget-content.fc-major')
    //                     // var trElements = document.querySelectorAll('.fc-resource-area.fc-widget-content tr[data-resource-id]')
    //                     tdElements.forEach(td => {
    //                         var tdDate = new Date(td.getAttribute('data-date'));
    //                         var isPast = tdDate < now || tdDate.getHours() < 7 || tdDate.getHours() >= 21
    //                         if (isPast) {
