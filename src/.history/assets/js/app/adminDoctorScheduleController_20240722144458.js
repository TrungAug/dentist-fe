app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.countDsByDoctor = (doctorId) => {
        console.log("doctorId",doctorId);

        for (let key in $scope.listDSByTimeRangesDB) {
            let filterData = $scope.listDSByTimeRangesDB[key];

            let filteredSchedules = filterData.filter(ds => {
                ds.doctor.doctorId === doctorId
            });
            let count=filteredSchedules.length
