app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getShift = (shiftId) => {
        if ($scope.isChangeSchedule) {
            var validDoctorUnavailable = $scope.isDoctorUnavailable = ($scope.selectedDoctors, $scope.selectedDates, shiftId)
            if (validDoctorUnavailable) {
                Swal.fire({
                    position: "top-end",
                    icon: "warning",
                    title: "Không thể đổi lịch, đã có khách đặt hẹn",
