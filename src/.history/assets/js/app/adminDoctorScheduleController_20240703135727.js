app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService) {
    ...
    $scope.crudDoctorSchedule = () => {
        ...
        $scope.changeSchedule = async (doctorSchedule) => {
            ...
            // await $scope.isDoctorUnavailable(doctorId, ds.date, ds.shift.shiftId).then(rs=>{
            //     isDoctorUnavailable=rs
            // })
            ...
            try {
                ...
                // Apply the change to isChangeSchedule and trigger the tab click
                $scope.$applyAsync(() => {
                    $scope.isChangeSchedule = true;
                    // Use AngularJS to trigger the click event
                    angular.element(document.getElementById('v-pills-register-schedule-tab')).triggerHandler('click');
                });
            } catch (error) {
