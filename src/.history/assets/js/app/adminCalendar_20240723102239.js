app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.validPatient = (patientId) => {
        //console.log("validPatient patientId",patientId);

        // viết thêm api lấy lịch hẹn theo bệnh nhân
        let params={
            date: new Date(),
            patientId:patientId
        }
        ...
        $http.get(url + '/appointment-by-patient',{params:params}).then(response => {
           ...
           // console.log("$scope.listAppByPatientDB",$scope.listAppByPatientDB);
        })


        // if($scope.checkPaintient.length > 0){
        //     filteredData = filteredData.filter(item => {
        //         return $scope.checkPaintient.includes(item.appointment.doctor.doctorId);
        //     });
        // }
    }
    ...
    $scope.initializeUIComponents = () => {
        ...
        $('#appointmentPatient').on('change', function () {
            $timeout(function () {
                ...
                $scope.validPatient($scope.formApp.patientId)
            });
        });
