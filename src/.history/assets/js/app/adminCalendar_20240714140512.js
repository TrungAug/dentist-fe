...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getListAppointmentBySatus = (listAppointmentDB, statusId) => {
        if (listAppointmentDB.length == 0) {
            ...
            let data = listAppointmentDB.forEach(app => {
                if(app.appointmentStatus.appointment_StatusId!=null){
                    return app.appointmentStatus.appointment_StatusId===statusId
                }    
            })
            let percent = (data.length/listAppointmentDB.length)*100
            return percent
        }
    }
