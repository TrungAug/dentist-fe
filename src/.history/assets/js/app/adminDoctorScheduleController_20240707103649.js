app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.isBookedShift = async (shiftId) => {
        if ($scope.selectedDates.length == 0) {
            $scope.selectedDates = $('.drgpickerSingle').val()
        }
        console.log("--------------------------------");
        console.log("$scope.selectedDates isBookedShift", $scope.selectedDates);
        // thay đổi ca
        console.log("shiftId isBookedShift", shiftId);
        // bác sĩ
        console.log("$scope.selectedDoctors isBookedShift", $scope.selectedDoctors);

        var result=await $scope.isDoctorUnavailable($scope.selectedDoctors, $scope.selectedDates, shiftId)
        console.log("result isBookedShift",result);
    }
    ...
    $scope.changeDoctorSchedule = () => {
        //các id cũ
        console.log("$scope.originalIds changeDoctorSchedule: ", $scope.originalScheduleIds);
