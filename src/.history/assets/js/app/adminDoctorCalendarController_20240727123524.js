app.controller('AdminDoctorCalendarController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.updateSelectedMedicine = (medicine) => {
        ...
        $scope.selectedMedicines.forEach(medicine =>{
            if (medicine.selectedFrequencies.length!=0){
               let timesOfDays= medicine.selectedFrequencies.forEach(frequency=>frequency.timesOfDay)
               console.log("timesOfDays",timesOfDays);
              // $scope.timesOfDayString=timesOfDays.join(' ,') || ""
            }
        })
        //console.log("$scope.timesOfDayString",$scope.timesOfDayString);
