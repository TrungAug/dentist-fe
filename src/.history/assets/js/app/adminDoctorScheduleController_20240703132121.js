app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service,TimezoneService) {
    ...
    $scope.crudDoctorSchedule = () => {
        ...
        $scope.isDoctorUnavailable = (doctorId, date, shiftId) => {
            ...
            var dateFmt=moment(date, "DD/MM/YYYY").toDate()
            var convertedDate = TimezoneService.convertToTimezone(dateFmt, defaultTimezone)
            $scope.processDoctorUnavailabilityByDoctor(doctorId).then(result => {
                filteredResults = result.filter(du => {
                    return du.date===convertedDate               
                })
            })
            isDoctorUnavailable = filteredResults.length > 0
            console.log("filteredResults", filteredResults)
            console.log("isDoctorUnavailable", isDoctorUnavailable)
        }

        // $scope.isDoctorUnavailable = (doctorId, date, shiftId) => {
        //     return new Promise((resolve, reject) => {
        //         let filteredResults = null;
        //         const dateFmt = moment(date, "DD/MM/YYYY").toDate();
        //         const convertedDate = TimezoneService.convertToTimezone(dateFmt, defaultTimezone);
        
        //         $scope.processDoctorUnavailabilityByDoctor(doctorId).then(result => {
        //             filteredResults = result.filter(du => {
        //                 const duDate = TimezoneService.convertToTimezone(new Date(du.date), defaultTimezone);
        //                 return duDate.getTime() === convertedDate.getTime() && du.shift.shiftId === shiftId;
        //             });
        
        //             const isDoctorUnavailable = filteredResults.length > 0;
        //             console.log("filteredResults", filteredResults);
        //             console.log("isDoctorUnavailable", isDoctorUnavailable);
        //             resolve(isDoctorUnavailable);
        //         }).catch(error => {
        //             console.error("Error processing doctor unavailability", error);
        //             reject(error);
        //         });
        //     });
        // }
        ...
        $scope.changeSchedule = (doctorSchedule) => {
            ...
            $scope.isDoctorUnavailable(doctorId,moment(ds.date).format('DD/MM/YYYY'),ds.shift.shiftId)

            console.log("$scope.formDoctorSchedule change", $scope.formDoctorSchedule);
