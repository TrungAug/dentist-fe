    ...
    $scope.initializeCalendarRegisterDoctorSchedule = () => {
        $scope.processDoctorScheduleAllDoctor().then(result => {
            ...
            var events = result.map(item => {
                return {
                    title: item.doctor ? item.doctor.fullName : null,
                    date: item.date ? `${item.date.split("T")[0]}` : null,
                    start: item.date ? `${item.date.split("T")[0]}` : null,
                };
            });
            ...
            var calendar = new FullCalendar2.Calendar(calendarEl, {
                ...
                select: function (arg) {
                    ...
                    var enDate = TimezoneService.convertToTimezone(arg.end, defaultTimezone)
                    $scope.endDate = moment(enDate, 'DD/MM/YYYY').subtract(1, 'days').format('DD/MM/YYYY')
                    $scope.$apply();
                    const btnEventDetailsListAppoinment = document.getElementById('btnRegisterDoctorSchedule');
                    btnEventDetailsListAppoinment.click();
                    console.log("arg", arg);
                    $scope.getSlectedDates(arg.startStr, arg.endStr)
