app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getDoctorByTimeRange = () => {
        ...
        for (let key in $scope.listDSByTimeRangesDB) {
            filterData = filterData.concat($scope.listDSByTimeRangesDB[key]);
        }
    
        // Lặp qua filterData để lấy uniqueDoctorIds và dsUniqueDoctors
        filterData.forEach(ds => {
            let doctorId = ds.doctor.doctorId + '-' + ds.doctor.fullName;
            if (!uniqueDoctorIds.includes(doctorId)) {
                uniqueDoctorIds.push(doctorId);
        ...
        // Lặp qua uniqueDoctorIds để lọc và tạo result
        uniqueDoctorIds.forEach(item => {
            let id = item.split('-')[0];
            
            // Lọc dsUniqueDoctors theo doctorId
            let filteredDs = dsUniqueDoctors.filter(ds => ds.doctor.doctorId === parseInt(id));
    
            // Tạo mảng [id, số lượng ds] và thêm vào result
            result.push([id, filteredDs.length]);
        });
