...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getOriginalData=(appointmentId) => {
        ...
        var getAppointmentPromises=$http.get(url+'/appointment-id/'+appointmentId).then((response) => {$scope.originalAppointment=response.data})
        var getOriginalDU=$http.get(url+'/doctorUnavailability-by-appid',{params:params}).then((response) => {$scope.originalDU=response.data})
        var getAriginalAS=$http.get(url+'/appointment-service-by-appid',{params:params}).then((response) => {$scope.originalAS=response.data})

        Promise.all(getAppointmentPromises,getOriginalDU,getAriginalAS)
    }
