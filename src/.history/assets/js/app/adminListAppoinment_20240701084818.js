app.controller('AdminListAppoinment', function ($scope, $http, $rootScope, $location, $timeout, TimezoneService, $route) {
    ...
    $scope.updateStatus = (appoinment, statusId) => {
        ...
        $scope.updateAppoinmentToDB(appoinment, statusId)
        if (statusUpdate) {
            switch (statusUpdate.status.toLowerCase()) {
                case 'đã hủy':
                    // Xử lý cho trường hợp 'đã hủy'
                    break;
                case 'không đến':
    ...
    $scope.updateAppoinmentToDB = (appoinment, statusId) => {
        var dataAppointmentRequestJSON=angular.toJson($scope.getDataAppointmentRequest(appoinment, statusId))
        var dataDoctorUnavailabilityRequest=angular.toJson($scope.getDataDoctorUnavailabilityRequest(appoinment))
        var dataAppointmentPatientRecordRequest=angular.toJson($scope.getDataAppointmentPatientRecordRequest(appoinment))
    ...
    $scope.getDataAppointmentRequest = (appoinment, statusId) => {
        console.log("appoinment getDataAppointmentRequest",appoinment);
        console.log("appoinment.appointment.patient.patientId",appoinment.appointment.patient.patientId);
        $scope.formAppointmentRequest = {
            patientId: appoinment.appointment.patient.patientId,
            appointmentStatus: statusId,
            appointmentType: appoinment.appointment.appointmentType.appointment_TypeId,
            doctorId: appoinment.appoinment.doctor.doctorId,
            dentalStaffId: appoinment.appoinment.dentalStaff.dentalStaffId,
            appointmentDate: appoinment.appoinment.appointmentDate,
            note: appoinment.appoinment.note,
            createAt: appoinment.appoinment.createAt,
            isDeleted: false,
            appointmentPatientRecord: appoinment.appoinment.appointmentPatientRecord.appointmentPatientRecordId
        }
        return $scope.formAppointmentRequest
    }

    $scope.getDataDoctorUnavailabilityRequest = (appoinment) => {
        $scope.formDoctorUnavailabilityRequest = {
            description: appoinment.appoinment.description,
            timeOfShiftId: appoinment.appoinment.timeOfShift.timeOfShiftId,
            appointmentId: appoinment.appoinment.appointment.appointmentId,
            date: appoinment.appoinment.date,
            isDeleted: false
        }
        return $scope.formDoctorUnavailabilityRequest
    }

    $scope.getDataAppointmentPatientRecordRequest = (appoinment) => {
        $scope.formAppointmentPatientRecordRequest = {
            patientId: appoinment.appoinment.patient.patientId,
            createAt: appoinment.appoinment.createAt,
