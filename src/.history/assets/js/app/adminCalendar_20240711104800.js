...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.isShiftAvailableCalendar=(tdDate)=>{
        var time=moment(tdDate.getTime()).format("HH:mm:ss")
        var checkData = $scope.listDuDB.filter(du=>du.timeOfShift.beginTime===time)
    ...
    $scope.getOriginalData = (appointmentId) => {
        ...
        var getOriginalDUPromise = $http.get(url + '/doctorUnavailability-by-appid', { params: params }).then((response) => { $scope.originalDU = response.data           
         })
                ...
                var calendar = new FullCalendar.Calendar(calendarEl, {
                    ...
                    datesRender: function (info) {
                        ...
                        tdElements.forEach(td => {
                            ...
                            //     td.remove()
                            // }
                            var isDu=$scope.isShiftAvailableCalendar(tdDate)
