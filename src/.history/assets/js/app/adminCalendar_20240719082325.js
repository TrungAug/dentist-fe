app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.isCheckTos = (tos, doctorId) => {
       
        let timeOfShiftId = tos.timeOfShiftId
        console.log("tos[0]",timeOfShiftId);
        // let isChecked = $scope.originalDU.some(oDu => oDu.timeOfShift.timeOfShiftId === timeOfShiftId && oDu.appointment.doctor.doctorId === doctorId)
        // tos.checked = isChecked;
        // return isChecked;
    ...
    $scope.getOriginalData = (appointmentId) => {
        ...
        let getOriginalDUPromise = $http.get(url + '/doctorUnavailability-by-appid', { params: params }).then((response) => {
            ...
            $scope.originalDU.forEach(du => {
                let item = [du.timeOfShift, du.appointment.doctor.doctorId]
    ...
    $scope.onChangeTimeOfShift = (tos, isUpdate, doctorId) => {
        ...
        console.log("$scope.arrSelectedTemp", $scope.arrSelectedTemp);
