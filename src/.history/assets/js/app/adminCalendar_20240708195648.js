...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.recommendShifts=(getTotalTime)=>{
        if (getTotalTime <= 30) {
            return "Số ca khuyến nghị: 1";
        } else if (getTotalTime <= 60) {
            return "Số ca khuyến nghị: 2";
        } else if (getTotalTime <= 90) {
            return "Số ca khuyến nghị: 3";
        } else {
            $scope.disableContinue = true
            return "Bạn cần liên hệ trực tiếp nhân viên để được hướng dẫn kỹ hơn";
        }
    }
