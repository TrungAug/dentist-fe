app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route, convertMoneyService) {
    ...
    $scope.getTotalPrice = () => {
        return $scope.selectedServices.reduce((total, service) => total + service.price * service.quantity, 0)
    }
    ...
    $scope.updateStatus = (app, status) => {
        ...
        const originalApp = app.appointment
        if (originalApp == null) {
            return
        }
        ...
        $scope.originalDate = moment(originalApp.appointmentDate, ('YYYY-MM-DD')).format('DD/MM/YYYY')
        $scope.comparasionDateEvent(originalApp.appointmentDate)
        $scope.selectedTosOriginal = [app.timeOfShift]
        $scope.formUpApp = {
            reExaminationDate: originalApp.appointmentPatientRecord.reExamination,
            currentCondition: originalApp.appointmentPatientRecord.currentCodition,
            appointmentDate: $scope.originalDate,
            fullName: originalApp.doctor.fullName,
            startTime: app.timeOfShift.beginTime,
            endTime: app.timeOfShift.endTime,
            patientId: originalApp.patient.patientId,
            appointmentStatus: originalApp.appointmentStatus ? originalApp.appointmentStatus.appointment_StatusId : 2,
            appointmentTypeId: originalApp.appointmentType.appointment_TypeId,
            createDate: originalApp.createAt,
    ...
    $scope.setupTabLApp = (appointmentId) => {
    ...
    $scope.exportPDF = (contentId, nameDocument, idNumber) => {
        ...
        $scope.$evalAsync(() => {
            document.fonts.ready.then(() => {
                ...
                // Create a temporary element for capturing content
                const tempElement = document.createElement('div');
                tempElement.innerHTML = pdfContent.innerHTML;
                tempElement.style.position = 'absolute';
                tempElement.style.left = '0px';
                tempElement.style.top = '0px';
                tempElement.style.width = '1000px';  // Adjust width as needed
                tempElement.style.height = 'auto';
                tempElement.style.visibility = 'hidden';  // Hide initially to prevent layout issues
                tempElement.style.background = 'white';
                document.body.appendChild(tempElement);

                // Function to capture content in multiple pages
                function captureContent(pageNumber = 1) {
                    // Set temporary element height to content height
                    tempElement.style.height = `${tempElement.scrollHeight}px`;
                    tempElement.style.visibility = 'visible';  // Make element visible for rendering

                    let contentHeight = tempElement.scrollHeight;
                    let maxIterations = 10; // Adjust as needed
                    let iteration = 0;

                    while (tempElement.scrollHeight > tempElement.clientHeight && iteration < maxIterations) {
                        contentHeight += 100; // Increase height incrementally
                        tempElement.style.height = `${contentHeight}px`;
                        iteration++;
                    }
                    ...
                    Promise.all(imagePromises).then(() => {
                        html2canvas(tempElement, {
                            ...
                            const imgData = canvas.toDataURL('image/png');
                            console.log(`Page ${pageNumber} imgData length:`, imgData.length);
                            ...
                            // Add page content to document definition
                            const pageContent = {
                                image: imgData,
                                width: 600,
                                style: {
                            ...
                            // Check if there's more content on next page
                            const hasMoreContent = tempElement.scrollHeight > tempElement.clientHeight;

                            if (hasMoreContent) {
                                // Clear content for next page capture
                                tempElement.innerHTML = '';
                                tempElement.style.height = 'auto';
                                tempElement.style.visibility = 'hidden';

                                // Capture content for the next page
                                captureContent(pageNumber + 1);
                            } else {
                                // No more content, create and download PDF
                                const documentDefinition = {
                                    content: [pageContent],
                                    defaultStyle: {
                                        font: "NimbusSans"
                ...
                // Start capturing content from the first page
                captureContent();
            });
        });
    };
