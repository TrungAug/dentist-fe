...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getOriginalData=(appointmentId) => {
        ...
        var getOriginalDU=$http.get(url+'/doctorUnavailability-by-appid/'+appointmentId).then((response) => {$scope.originalDU=response.data})
        var getAriginalAS=$http.get(url+'/appointment-service-by-appid/'+appointmentId).then((response) => {$scope.originalAS=response.data})
