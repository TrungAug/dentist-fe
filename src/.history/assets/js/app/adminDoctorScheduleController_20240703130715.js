app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service) {
    ...
    $scope.crudDoctorSchedule = () => {
        ...
        $scope.isDoctorUnavailable = (doctorId, dates, shiftIds) => {
            var isDoctorUnavailable = false
            var filteredResults =null
            $scope.processDoctorUnavailabilityByDoctor(doctorId).then(result => {
                filteredResults = result.filter(du => {
                    const dateMatches = dates.includes(moment(du.date).format('DD/MM/YYYY'));
                    const shiftMatches = shiftIds.includes(du.shift.shiftId);
