app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.crudDoctorSchedule = () => {
        ...
        $scope.getDataChangeDoctorSchedule = async (date, shiftId, scheduleOrigin) => {
            var doctorId=scheduleOrigin.doctor.doctorId
            var data=null
            await $scope.processDoctorScheduleByDoctor(doctorId).then(result => {
                data=result.filter(item=>item.date===date&&item.shift.shiftId===shiftId)               
            })
        ...
        $scope.changeDoctorSchedule = async () => {
            console.log("$scope.formDoctorSchedule old", $scope.formDoctorSchedule);
            var scheduleOrigin = $scope.formDoctorSchedule
            var selectedDate = $('#dateOfDoctorSchedule').val()
            var dates = $scope.getDateArray(selectedDate)[0]
            var shiftIds = selecteShiftdValues && selecteShiftdValues.length > 0 ? selecteShiftdValues[0] : $scope.formDoctorSchedule.shiftId

            await $scope.getDataChangeDoctorSchedule(dates, shiftIds, scheduleOrigin)

            // $scope.formDoctorSchedule = {
            //     doctorScheduleId: scheduleOrigin.doctorScheduleId,
            //     date: TimezoneService.convertToTimezone(moment(dates, 'DD/MM/YYYY').toDate(), defaultTimezone),
            //     shiftId: shiftIds,
            ...
            //     updateAt: new Date()
            // }         
            // console.log("$scope.formDoctorSchedule new", $scope.formDoctorSchedule);
        }
