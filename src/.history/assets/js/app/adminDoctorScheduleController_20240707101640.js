app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getDoctorShiftsExcludingDeleted = async (date, doctorId) => {
        // var params = {
        //     date: date,
        //     doctorId: doctorId
        // }
        ...
        try {
            ...
            const response = await $http.get(url + '/get-doctor-shifts-excluding-deleted', { params: params });
            $scope.listDoctorShiftsDB = response.data;
           ...
           var rs= await $scope.isDoctorUnavailable(doctorId, date, $scope.selectedShifts);
           if(rs) {
            $scope.validShifts = rs.filteredResults
           }
           console.log("$scope.validShifts",$scope.validShifts);
        } catch (error) {
            console.error('Đã xảy ra lỗi khi lấy thông tin ca trực của bác sĩ:', error);
        }
    }
    ...
    $scope.isDoctorUnavailable = (doctorId, date, shiftIds) => {
        return new Promise((resolve, reject) => {
            ...
            $scope.processDoctorUnavailabilityByDoctor(doctorId).then(result => {
                ...
                var isDoctorUnavailable = filteredResults && filteredResults.length > 0;
                resolve({ isDoctorUnavailable: isDoctorUnavailable, filteredResults: filteredResults });
            }).catch(err => {
                reject(err);
            });
        });
    ...
    $scope.changeDoctorSchedule = () => {
        ...
        $scope.originalIds.forEach(function (idString) {
