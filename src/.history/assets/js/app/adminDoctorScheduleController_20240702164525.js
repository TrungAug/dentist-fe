app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout,$window,processSelect2Service) {
    ...
    $scope.getListDoctorScheduleByDoctor = () => {
        $scope.processDoctorScheduleByDoctor().then(result => {
            $scope.listDoctorUnavailabilityByDoctorDB = result
            // console.log("$scope.listDoctorUnavailabilityByDoctorDB", $scope.listDoctorUnavailabilityByDoctorDB);
            $timeout(() => {
    ...
    $scope.processDoctorScheduleByDoctor = () => {
        return new Promise((resolve, reject) => {
            $http.get(url + '/doctor-schedule-by-doctor-id/'+doctorId).then((response) => { 
                console.log("response unsorrt ",response.data);
                var result=response.data.sort((item1,item2) => {
                    var date1 = moment(new Date(item1.date.split('T')[0])).format("DD/MM/YYYY")
                    var date2 = moment(new Date(item2.date.split('T')[0])).format("DD/MM/YYYY")
                    return date2 - date1
                })           
