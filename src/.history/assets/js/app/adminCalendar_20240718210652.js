app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.originalDU = []
    $scope.originalAS = []
    $scope.originalselectedTimeOfShiftUp = []
    ...
    $scope.isShiftAvailable = (timeOfShiftId, isUpdate,doctorId) => {
        console.log("doctorId",doctorId);
        console.log("$scope.listTOSFilter",$scope.listTOSFilter);
        let checkData = $scope.listTOSFilter.filter(tos => tos[1].timeOfShiftId === timeOfShiftId&&tos[0].doctor.doctorId===doctorId);

        if (isUpdate) {
            let isChecked = $scope.originalselectedTimeOfShiftUp.map(tos => tos.timeOfShiftId);
            checkData = checkData.filter(tos => !isChecked.includes(tos[1].timeOfShiftId));
            console.log("$scope.originalselectedTimeOfShiftUp", $scope.originalselectedTimeOfShiftUp);
        }
    ...
    $scope.getOriginalData = (appointmentId) => {
        ...
        let getOriginalDUPromise = $http.get(url + '/doctorUnavailability-by-appid', { params: params }).then((response) => {
            ...
            $scope.originalDU.forEach(du => {
                $scope.originalselectedTimeOfShiftUp.push(du.timeOfShift)
            })
        })
    ...
    $scope.initializeCalendarAppointment = () => {
        ...
        const loadDoctorUnavailability = () => {
            ...
            $scope.processDoctorUnavailability().then(dataDu => {
                let checkStatus = ['hoàn thành', 'đã hủy', 'không đến', 'hoãn']
                dataDu = dataDu.filter(du => {
                    let isAuth = checkStatus.includes(du.appointment.appointmentStatus.status.toLowerCase())
    ...
    $scope.getDoctorFilter = (doctorId) => {
        let index = $scope.selectedDoctorLApp.indexOf(doctorId)
        if (index === -1) {
            $scope.selectedDoctorLApp.push(doctorId)
        } else {
            $scope.selectedDoctorLApp.splice(index, 1)
        }
