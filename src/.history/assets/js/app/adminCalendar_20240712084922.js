            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                select: function (arg) {
                    var hasEvent = calendar.getEvents().some(event => {
                        // var conditionOne=event.start < arg.end && arg.start < event.end &&
                        // event.getResources().some(resource => resource.id === arg.resource.id)
                        // var conditinTwo=(event.start > arg.start || arg.end > event.end) &&
                        // event.getResources().some(resource => resource.id === "")

                        //return conditionOne
                        return (
                            event.start < arg.end && arg.start < event.end &&
                            event.getResources().some(resource => resource.id === arg.resource.id)
                        ) || ((event.start > arg.start || arg.end > event.end) &&
                         event.getResources().some(resource => resource.id === ""));
                    });
