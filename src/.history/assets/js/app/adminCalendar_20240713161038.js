...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.updateAppointment = () => {
        ...
        Promise.all([
            // handleApiRequest(deleteUrl, postUrl, putUrl, data, idKey)
            handleApiRequest("/soft-delete-appointment-patient-record", "/appointment-patient-record", "/appointment-patient-record", reqAprData, aprKey),
            handleApiRequest("/soft-delete-appointment", "/appointment", "/appointment", reqAppData, appKey),
            handleApiRequest("/soft-delete-doctorUnavailability", "/doctorUnavailability", "/doctorUnavailability", reqDuData, duKey),
            handleApiRequest("/soft-delete-appointment-service", "/appointment-service", "/appointment-service", reqAsData, asKey)
        ]).then(result => {
            Swal.fire({
                title: "Thành công!",
