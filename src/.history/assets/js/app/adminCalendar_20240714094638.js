...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.onChangeTimeOfShift = (tos, isUpdate, doctorId) => {
        ...
        //     return arrSelectedTemp.find(item => item[1] === doctorId)
        // }
        // let isAuth = findDoctor(doctorId)
        // console.log("isAuth", isAuth);
        if(arrSelectedTemp.length>0){
            arrSelectedTemp.forEach(item => console.log("item[1]===doctorId",item[1]===doctorId))
        }
        if (tos.checked) {

            arrSelectedTemp.push(item)
            console.log("arrSelectedTemp", arrSelectedTemp);
        }
        // if (tos.checked) {
        //     if (isUpdate) {
