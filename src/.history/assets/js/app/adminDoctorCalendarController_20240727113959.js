app.controller('AdminDoctorCalendarController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
        ...
        selectedMedicines.forEach(medicine => {
            medicine.selectedFrequencies.forEach(frequency=>{
                let prescriptionMedicinesRequest = {
                    frequency:frequency.timesOfDay,
    ...
    $scope.doctorCreatePrescription= async() => {
        ...
        let prescriptionId = responsePrescription.prescriptionId !== null ? responsePrescription.prescriptionId : null;

        let prescriptionMedicineRequest=$scope.generatePrescriptionMedicineRequest($scope.selectedMedicines, prescriptionId,description)

        // $http.post(url+'/prescriptions',prescriptionDataRequest[0]).then(response => {
        //     let prescriptionId=response.data.prescriptionId
        //     console.log(response.data);
        // })

        // console.log("description",description);
        // console.log("$scope.selectedTreatmentDuration",$scope.selectedTreatmentDuration);
        // console.log("$scope.aprId",$scope.aprId);
        // console.log("$scope.selectedMedicines", $scope.selectedMedicines);
    }
    ...
    $scope.initializeUIComponentsModal = () => {
        ...
        $('#treatmentDurationId').on('change', function () {
            $timeout(function () {
                ...
                console.log("$scope.treatmentDurationQuantity", $scope.treatmentDurationQuantity);
                $scope.getListMedicine()
            });
        });
