...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.saveAppointment = async () => {
        ...
        var response = await $http.post(url + '/appointment-patient-record', dataAPRReq)
        $scope.appointmentRequest.appointmentPatientRecord = response.data.appointmentPatientRecordId
       // $scope.appointmentRequest.appointmentPatientRecord=12
        var dataAppoinmentReq=$scope.appointmentRequest
        var dataArrayDUReq = $scope.generateDoctorUnavailabilityRequest($scope.selectedTimeOfShift,23)
        var dataArrayServiceReq =$scope.generateAppointmentServiceRequest($scope.selectedServices,23)
        
        console.log("dataAPRReq",dataAPRReq);
        console.log("dataAppoinmentReq",dataAppoinmentReq);
