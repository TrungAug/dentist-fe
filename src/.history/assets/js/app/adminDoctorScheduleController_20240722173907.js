app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getDoctorScheduleAllDoctor = () => {
        $http.get(url + '/doctor-schedule-except-deleted').then((response) => {
          ...
          //  console.log("response.data kuku",response.data);
            console.log(" $scope.selectedDateFilterTable", $scope.selectedDateFilterTable);
            if($scope.selectedDateFilterTable.length > 0) {
                $scope.listDoctorScheduleAllDoctorDB=$scope.listDoctorScheduleAllDoctorDB.filter(item=>{
                    console.log("aaaaaaaaaaaa",moment(item.date).format("YYYY-MM-DD"));
    ...
    $scope.getDoctorScheduleByTimeRange = (startStr, endStr, paramsFiler) => {
        ...
        if (!paramsFiler) {
            ...
            $http.get(url + '/map-date-ds-by-time-range', { params: params }).then(response => {
                ...
                console.log("$scope.isShowDetail",$scope.isShowDetail);
