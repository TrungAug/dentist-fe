app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    //                 selectable: true, 
    //                 select: function(start, end, view, event) {
    //                     // Kiểm tra nếu phím Ctrl được giữ
    //                     var selectedDates = []
    //                     if (event.ctrlKey) {

    //                       // Thêm ngày được click vào danh sách ngày đã chọn
    //                       // (Bạn cần tự viết logic để lưu trữ danh sách này)
    //                       selectedDates.push(start.format('YYYY-MM-DD'));
    //                       console.log("selectedDates",selectedDates);
    //                     } else {
    //                       // Xóa danh sách ngày đã chọn
    //                       selectedDates = [];
    //                       // Chọn vùng từ 'start' đến 'end'
                ...
                var calendar = new FullCalendar.Calendar(calendarEl,
                    {
                        ...
                        header:
                        {
                            left: 'today, prev, next',
                            center: 'title',
                            right: 'timeGridDay,dayGridMonth,listMonth',
                        },
