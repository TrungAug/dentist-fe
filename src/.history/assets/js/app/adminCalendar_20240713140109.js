...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.processDataUpdate = (originalArr, reqArr) => {
        var oL = originalArr.length
        var rL = reqArr.length
        var oUp = []
        var oAdd = []
        var oDel = []
        var rChangeUp = []
        if (oL > rL) {
            //dữ liệu db
            oUp = originalArr.slice(0, rL)
            rChangeUp=reqArr
            oDel = originalArr.slice(rL)
        } else if(oL < rL) {
            // dữ liệu req
            // originalArr[i1,i2]
            //reqArr[u1,u2,u3]
            oUp = originalArr
            rChangeUp=reqArr.slice(0, oL)
            oAdd = reqArr.slice(oL)
        }else{
            oUp = reqArr
        }
        // console.log("oUp", oUp);
        // console.log("oAdd", oAdd);
        // console.log("oDel", oDel);
        return {
            updateArr:[[oUp],[rChangeUp]],
            deleteArr:[oDel],
            postArr:[oAdd]
        }
    }
    ...
    $scope.updateAppointment = () => {
        ...
        var originalApr = $scope.originalAPR
        var originalApp = $scope.originalAppointment
        var originalDu = $scope.originalDU
        var originalAs = $scope.originalAS

        var aprId = originalApr.appointmentPatientRecordId
        var appId = originalApp.appointmentId
        var duId = []
        var apsId = []
        ...
        var reqApr = angular.toJson($scope.appointmentPatientRecordRequest)
        var reqApp = angular.toJson($scope.appointmentRequest)
        var reqDu = $scope.generateDoctorUnavailabilityRequest($scope.selectedTimeOfShiftUp, appId, isUpdate)
        var reqAs = $scope.generateAppointmentServiceRequest($scope.selectedServicesUp, appId, isUpdate)
        ...
        //Cập 
        var reqAprData = $scope.processDataUpdate(originalApr, reqApr)
        var reqAppData = $scope.processDataUpdate(originalApp, reqApp)
        var reqDuData = $scope.processDataUpdate(originalDu, reqDu)
        var reqAsData=$scope.processDataUpdate(originalAs, reqAs)

        console.log("reqAprData",reqAprData);
        console.log("originalApr",originalApr);
        console.log("reqApr",reqApr);
