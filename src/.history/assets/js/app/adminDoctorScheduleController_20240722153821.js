app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getDoctorScheduleByTimeRange = (startStr, endStr, paramsFiler) => {
        ...
        if (!paramsFiler) {
            ...
            // $http.get(url + '/ds-by-time-range', { params: params }).then(response => {
            ...
            $http.get(url + '/map-date-ds-by-time-range', { params: params }).then(response => {
                //click chọn ngày để xem thống kê
                $scope.listDSByTimeRangesDB = response.data
                // console.log("$scope.listDSByTimeRangesDB",$scope.listDSByTimeRangesDB);
            })
        }
    }


    $scope.countShiftByTimeRange = (listDSByTimeRangesDB, shift) => {
        let filterData = []
        for (let key in listDSByTimeRangesDB) {
            filterData = listDSByTimeRangesDB[key]
        }
        filterData = filterData.filter(ds => ds.shift.shiftId === shift.shiftId)
        let dsUniqueDoctors = []
        let uniqueDoctorIds = []

        filterData.forEach(ds => {
            let doctorId = ds.doctor.doctorId;
            if (!uniqueDoctorIds.includes(doctorId)) {
                uniqueDoctorIds.push(doctorId);
                dsUniqueDoctors.push(ds);
            }
        });
        return [dsUniqueDoctors.length, dsUniqueDoctors]
    }

    $scope.getHourWorking = (listDSByTimeRangesDB, doctorId) => {
        const currentDate = new Date()
        let filterData = []
        for (let key in listDSByTimeRangesDB) {
            filterData = listDSByTimeRangesDB[key]
        }
        filterData = filterData.filter(ds => ds.doctor.doctorId === doctorId)
        let pastArr = filterData.filter(ds => new Date(ds.date) < currentDate)
        let totalShift = filterData.length
        let totalPastShift = pastArr.length
        let hours = totalPastShift * 4
        return [hours, totalShift, totalPastShift]
    }

    // $scope.getDoctorByTimeRange = () => {

    //     let filterData = []
    //     let uniqueDoctorIds = []
    //     for (let key in $scope.listDSByTimeRangesDB) {
    //         filterData = $scope.listDSByTimeRangesDB[key]
    //     }
    ...
    $scope.getDoctorByTimeRange = () => {
        ...
        // Lặp qua mỗi bác sĩ trong mảng uniqueDoctorIds
        uniqueDoctorIds.forEach(doctorInfo => {
            let [doctorId, fullName] = doctorInfo.split('-');
           

            // Đếm số lượng ca làm việc của bác sĩ
            for (let key in $scope.listDSByTimeRangesDB) {
                $scope.listDSByTimeRangesDB[key].forEach(ds => {
                    let count = 0;
                    if (ds.doctor.doctorId === doctorId && ds.doctor.fullName === fullName) {
                        count++;
                        
                    }
                });
                result.push([doctorId, count]);
            }

            // Thêm kết quả vào mảng result dưới dạng mảng con [doctorId, count]
