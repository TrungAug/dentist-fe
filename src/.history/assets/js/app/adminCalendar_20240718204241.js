app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.isShiftAvailable = (timeOfShiftId,isUpdate) => {
        let checkData=[]
        if (isUpdate) {
            let isChecked=[]
            $scope.originalselectedTimeOfShiftUp.forEach(tos =>{
                isChecked.push(tos.timeOfShiftId)
            })
            checkData= $scope.listTOSFilter.filter(tos => tos[1].timeOfShiftId === timeOfShiftId)
