app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getListDoctorUnavailabilityAllDoctor = () => {
        $scope.processDoctorUnavailabilityAllDoctor().then(result => {
            ...
            console.log("result huhu",result);   
            $scope.listAppointmentDBFromDu = []
            result.forEach(rs=>{
                console.log("rs.appointment",rs.appointment);
                $scope.listAppointmentDBFromDu.push(rs.appointment)
                const index = $scope.listAppointmentDBFromDu.findIndex(a => a.appointmentId === rs.appointment.appointmentId)
                if(index!=-1){
                    $scope.listAppointmentDBFromDu.splice(index, 1);
                }
