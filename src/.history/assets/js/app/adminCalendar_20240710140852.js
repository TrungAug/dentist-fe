    ...
    $scope.initializeCalendarAppointment = () => {
        $scope.getColorForStatusId = (status) => {
            ...
            switch (status.toLowerCase()) {
                case "đã xác nhận":
                    baseColor = 'rgba(92, 184, 92, 0.7)'
                case "đã đặt":
                    baseColor = 'rgba(169, 169, 169, 0.7)'
                case "đang diễn ra":
                    baseColor = 'rgba(0, 123, 255, 0.7)'
                case "đã hủy":
                    baseColor = 'rgba(217, 83, 79, 0.7)'
                case "không đến":
                    baseColor = 'rgba(240, 173, 78, 0.7)'
                case "hoãn":
                    baseColor = 'rgba(255, 193, 7, 0.7)'
                case "hoàn thành":
                    baseColor = 'rgba(0, 166, 90, 0.7)'
                default:
                    baseColor = 'rgba(169, 169, 169, 0.7)'
            }
            if (!isAvailable) {
                baseColor = 'rgba(255, 0, 0, 0.5)'; // Màu đỏ nhạt cho không khả dụng
            }
            return baseColor;
        }
