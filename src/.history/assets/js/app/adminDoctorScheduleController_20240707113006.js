app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.changeDoctorSchedule = async () => {
        ...
        var validChanges = await $scope.isDoctorUnavailable($scope.selectedDoctors, moment($scope.originalDate, 'YYYY-MM-DD').format('DD/MM/YYYY'), $scope.selectedShifts)

        console.log("validChanges", validChanges);

        if (validChanges.isBooked) {
            console.log("$scope.selectedDates", $scope.selectedDates);
            console.log("$scope.originalDate", moment($scope.originalDate, 'YYYY-MM-DD').format('DD/MM/YYYY'));
            console.log("check",moment($scope.originalDate, 'YYYY-MM-DD').format('DD/MM/YYYY')==$scope.selectedDates);
            if ($scope.originalDate != $scope.selectedDates) {
                Swal.fire({
                    position: "top-end",
                    icon: "warning",
                    title: "Ngày làm đã có khách đặt lịch hẹn.",
                    showConfirmButton: false,
                    timer: 1500
                });
                $scope.formDoctorSchedule.date = $scope.originalDate
                return
            }
        }
