app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.isShiftSelected = (shiftId) => {
        return $scope.listDoctorShiftsDB.some(function (shift) {
    ...
    $scope.getDoctorShiftsExcludingDeleted = (date, doctorId) => {
        var params = {
            date: date,
            doctorId: doctorId
        }
        $http.get(url + '/get-doctor-shifts-excluding-deleted', { params: params }).then(response => {
            ...
            response.data.forEach(item => {
                $scope.listDoctorShiftsDB =item[0]
                $scope.formDoctorSchedule.doctorScheduleId=item[1]
            });
            // $scope.listDoctorShiftsDB = response.data[0]
            // $scope.formDoctorSchedule.doctorScheduleId=response.data[1]
            console.log("$scope.listDoctorShiftsDB", $scope.listDoctorShiftsDB);
            console.log("$scope.formDoctorSchedule.doctorScheduleId", $scope.formDoctorSchedule.doctorScheduleId);
        })
    }
    ...
    $scope.createDoctorSchedule = () => {
        // Swal.fire({
        //     title: "Thành công!",
        //     html: "Đã đăng ký lịch làm việc cho ngày ",
        //     icon: "success"
        // })
        ...
        $scope.processDoctorScheduleAllDoctor().then(result => {
            ...
            var calendar = new FullCalendar2.Calendar(calendarEl, {
                ...
                eventClick: function (arg) {
                    ...
                    // document.getElementById('eventDetailsDoctorScheduleBody').innerHTML = eventDetails;
                    ...
                    $scope.formDoctorSchedule = {
                        doctorScheduleId: -1,
                        date: moment(arg.event.startStr, 'YYYY-MM-DD').format('DD/MM/YYYY'),
                        shiftId: '',
                        doctorId: arg.event.extendedProps.doctor.doctorId,
                        available: true,
                        createAt: new Date(),
                        updateAt: ''
                    }

                    $scope.getDoctorShiftsExcludingDeleted(arg.event.startStr, arg.event.extendedProps.doctor.doctorId)
                    $scope.$apply();
