app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.processChangeDoctorSchedule = (originalShifts,selectedShifts,isChangeDate) => {
        ...
        if(changeDate){
            if (originalShiftsLenght > selectedShiftsLenght) {
                ...
                deleteShifts=originalShifts.filter(id.split('-')[0]=>selectedShifts.includes())
    ...
    $scope.changeDoctorSchedule = async () => {
        ...
        var originalShiftsLenght = $scope.originalScheduleIds.length
        var selectedShiftsLenght = $scope.selectedShifts.length
        if (validChanges.isBooked) {
            ...
            //trường hợp ngày đã có lịch bận thì không cho đổi ngày, chỉ sét đến sự thay đổi ca
            //Ta có được các ca làm việc cũ , nếu lenght < $scope.selectedShifts.length thì kiểm tra ca thiếu và  bị xóa, ngược lại thì thêm ca.
