app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.initializeUIComponents = () => {
        ...
        $('.drgpicker-filter').on('apply.daterangepicker', function (ev, picker) {
            ...
            // $http.get(url + '/doctor-schedule-by-time-range', { params: paramsFiler }).then(response => {
            //     $scope.listDSFilterDB = response.data
            // })
            // console.log("$scope.listDSFilterDB",$scope.listDSFilterDB);
        });
    }
