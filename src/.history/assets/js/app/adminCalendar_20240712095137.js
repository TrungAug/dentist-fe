        ...
        const renderCalendar = () => {
            ...
            calendar = new FullCalendar.Calendar(calendarEl, {
                ...
                select: function (arg) {
                    ...
                    var invalidTimeRanges = calendar.getEvents().filter(event => {
                        return (
                            event.id === "" &&
                            event.getResources().some(resource => {
                                return resource != null && resource.id === arg.resource.id;
                            })&& moment(event.start).isSame($scope.fullCalendarDate, 'day')
                        );
                    });
                    
                    var isInvalidTimeRange = invalidTimeRanges.some(event => {
                        return (
                            arg.start < event.start || arg.end > event.end ||
                            (arg.start >= event.start && arg.start < event.end && arg.end > event.end) ||
                            (arg.end <= event.end && arg.end > event.start && arg.start < event.start)
                        );
                    });
                    ...
                    if (isInvalidTimeRange) {
                        Swal.fire({
                            position: "top-end",
                            icon: "warning",
                            title: "Thời gian chọn không hợp lệ với sự kiện này!",
                            showConfirmButton: false,
                            timer: 1500
                        });
                        calendar.unselect();
                ...
                //     $scope.resourceId = arg.resource.id
                // },
