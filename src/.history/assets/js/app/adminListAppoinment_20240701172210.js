app.controller('AdminListAppoinment', function ($scope, $http, $rootScope, $location, $timeout, TimezoneService, $route) {
    ...
    $scope.updateStatus = (appointment, statusId) => {
        console.log("appointment update", appointment);
        console.log(" $scope.listAppointmentStatusBD update", $scope.listAppointmentStatusBD);
        var oldStatusId = appointment.appointment.appointmentStatus.appointment_StatusId
                ...
                case 'hoãn':
                    Swal.fire({
                        ...
                        if (rs.dismiss === Swal.DismissReason.cancel) {
                            appointment.appointment.appointmentStatus.appointment_StatusId = oldStatusId
                            console.log("appointment.appointment.appointmentStatus.appointment_StatusId",appointment.appointment.appointmentStatus.appointment_StatusId);
                            $scope.$apply()
                        } else if (rs.isConfirmed) {
                            var isDeleted = true
                            var currentCondition = ""
                            var reExamination = ""
                            $scope.updateAppoinmentToDB(appointment, statusId, isDeleted, currentCondition, reExamination)
                        }
                    })
                    break;
                case 'đang diễn ra':
    ...
    $scope.updateAppoinmentToDB = async (appointment, statusId, isDeleted, currentCondition, reExamination) => {
        try {
            var dataAppointmentRequestJSON = angular.toJson($scope.getDataAppointmentRequest(appointment, statusId, isDeleted))
            var dataDoctorUnavailabilityRequest = angular.toJson($scope.getDataDoctorUnavailabilityRequest(appointment, isDeleted))
            var dataAppointmentPatientRecordRequest = angular.toJson($scope.getDataAppointmentPatientRecordRequest(appointment, isDeleted, currentCondition, reExamination))

            console.log("dataAppointmentRequestJSON", dataAppointmentRequestJSON);

            const promises = [
                $http.put(url + '/appointment-patient-record/' + appointment.appointment.appointmentPatientRecord.appointmentPatientRecordId, dataAppointmentPatientRecordRequest),
                $http.put(url + '/appointment/' + appointment.appointment.appointmentId, dataAppointmentRequestJSON),
                $http.put(url + '/doctorUnavailability/' + appointment.doctorUnavailabilityId, dataDoctorUnavailabilityRequest)
            ]
    ...
    $scope.getDataAppointmentRequest = (appointment, statusId, isDeleted) => {
        $scope.formAppointmentRequest = {
            patientId: appointment.appointment.patient ? appointment.appointment.patient.patientId : null,
            appointmentStatus: statusId,
            appointmentType: appointment.appointment.appointmentType.appointment_TypeId,
            doctorId: appointment.appointment.doctor ? appointment.appointment.doctor.doctorId : null,
            dentalStaffId: appointment.appointment.dentalStaff ? appointment.appointment.dentalStaff.dentalStaffId : null,
            appointmentDate: TimezoneService.convertToTimezone(moment(appointment.appointment.appointmentDate).toDate(), defaultTimezone),
            note: appointment.appointment.note,
            createAt: appointment.appointment.createAt,
            isDeleted: isDeleted,
            appointmentPatientRecord: appointment.appointment.appointmentPatientRecord.appointmentPatientRecordId
        }
        return $scope.formAppointmentRequest
    }

    $scope.getDataDoctorUnavailabilityRequest = (appointment, isDeleted) => {
        $scope.formDoctorUnavailabilityRequest = {
            description: appointment.appointment.description,
            timeOfShiftId: appointment.timeOfShift.timeOfShiftId,
    ...
    $scope.getDataAppointmentPatientRecordRequest = (appointment, isDeleted, currentCondition, reExamination) => {
        $scope.formAppointmentPatientRecordRequest = {
            patientId: appointment.appointment.patient ? appointment.appointment.patient.patientId : null,
            createAt: appointment.appointment.createAt,
