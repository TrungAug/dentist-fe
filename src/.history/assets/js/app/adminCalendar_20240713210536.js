...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.initializeUIComponentsModal = () => {
        ...
        $scope.$watch('formUpApp.appointmentStatus', function (newValue) {
            var selectedOption = $scope.listAppointmentStatusBD.find(item => item.appointment_StatusId === newValue);
            if (selectedOption) {
                let status = selectedOption.status.toLowerCase()
                switch (status) {
                    case 'đã hủy':
                    case 'không đến':
                    case 'hoãn':
                    case 'hoàn thành':
                        $('#appointmentStatusUp').prop('disabled', true).trigger('change');
                    default:
                        $('#appointmentStatusUp').prop('disabled', false).trigger('change');
                }
            }       
        });
    }
