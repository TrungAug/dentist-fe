    ...
    $scope.initializeCalendarAppointment = () => {
        ...
        const loadDoctorUnavailability = () => {
            ...
            $scope.getAllDoctorScheduleExceptDeleted().then(dataDs => {
                dataDs.forEach(ds => {
                    let startTime = moment(ds.date + 'T' + ds.shift.beginTime);
                    let endTime = moment(ds.date + 'T' + ds.shift.endTime);
                    // console.log("startTime",startTime);
                    // console.log("endTime",endTime);
                    while (startTime.isBefore(endTime)) {
                        let event = {
                            id: "",
                            start: startTime.format(), // Lưu ý định dạng thời gian theo ISO 8601
                            end: startTime.add(30, 'minutes').format(), // Thêm 30 phút vào thời gian bắt đầu để có thời gian kết thúc
