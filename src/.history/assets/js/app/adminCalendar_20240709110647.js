...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.appointmentRequest = {
        ...
        note: $scope.formApp.dentalIssuesId,
        createAt: TimezoneService.convertToTimezone(moment(new Date()).format("DD/MM/YYYY"), defaultTimezone),
        isDeleted: false
    }
    ...
    $scope.onChangeTimeOfShiftId = (timeOfShiftId) => {
        $scope.formApp.timeOfShiftId = timeOfShiftId
    }
    ...
    $scope.initializeUIComponents = () => {
        ...
        $('#dental-issues').on('change', function () {
            $timeout(function () {
                ...
                var getDentalIssueName = $scope.listDentalIssueDB.filter(di => {
                    if (selecteDentalIssues.includes(di.dentalIssuesId)) {
                        return di.name
                    }
                })
                console.log("getDentalIssueName",getDentalIssueName);
            });
        });
    }
