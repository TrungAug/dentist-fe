app.controller('AdminListAppoinment', function ($scope, $http, $rootScope, $location, $timeout, TimezoneService, $route) {
    ...
    $scope.updateStatus = (appointment, statusId) => {
        ...
        if (statusUpdate) {
            switch (statusUpdate.status.toLowerCase()) {
                case 'đã hủy':
                    $scope.formAppointmentRequest.isDeleted=true
                    $scope.formDoctorUnavailabilityRequest.isDeleted=true
                    $scope.formAppointmentPatientRecordRequest.isDeleted=true
                    $scope.updateAppoinmentToDB(appointment, statusId)
                    break;
                case 'không đến':
                    // Xử lý cho trường hợp 'không đến'
                    break;
                case 'hoãn':
                    // Xử lý cho trường hợp 'hoãn'
                    break;
                case 'đang diễn ra':
                    // Xử lý cho trường hợp 'đang diễn ra'
                    break;
                case 'đã xác nhận':
                    // Xử lý cho trường hợp 'đã xác nhận'
                    break;
                case 'đã đặt':
                    // Xử lý cho trường hợp 'đã đặt'
                    break;
                case 'hoàn thành':
                    // Xử lý cho trường hợp 'hoàn thành'
                    break;
                default:
    ...
    $scope.getDataAppointmentRequest = (appointment, statusId,isDeleted) => {
        console.log("appointment getDataAppointmentRequest", appointment);
        console.log("appointment.appointment.doctor.doctorId", appointment.appointment.doctor.doctorId);
        $scope.formAppointmentRequest = {
