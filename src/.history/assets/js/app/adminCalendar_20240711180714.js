...
app.controller('AdminCalendar', function ($scope, $http, $rootScope, $location, $timeout, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.getAllDoctorScheduleExceptDeleted = () => {
        return new Promise((resolve, reject) => {
            // /doctor-schedule-except-deleted
            $http.get(url + '/doctor-schedule-and-tos').then(response => {
                resolve(response.data)
            }).catch(err => reject(err))
        })
    }
    ...
    $scope.initializeCalendarAppointment = () => {
        ...
        const loadDoctorUnavailability = () => {
            ...
            $scope.getAllDoctorScheduleExceptDeleted().then(dataDs => {  
               // console.log("doctor-schedule-and-tos",dataDs);        
                dataDs.forEach(ds => {
                    let event = {
                        id: "",
                        start: ds[0].date.split("T")[0] + "T" + ds[1].beginTime,
                        end: ds[0].date.split("T")[0] + "T" + ds[1].endTime,
                        resourceId: ds[0].doctor.doctorId,
                        color: "#9fecf6"
                    };
                    eventArr.push(event);
                })
