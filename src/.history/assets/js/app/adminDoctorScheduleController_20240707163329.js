app.controller('AdminDoctorScheduleController', function ($scope, $http, $rootScope, $location, $timeout, $window, processSelect2Service, TimezoneService, $route) {
    ...
    $scope.processChangeDoctorSchedule = (originalShifts, selectedShifts,selectedDates, isChangeDate) => {
    ...
    $scope.changeDoctorSchedule = async () => {
        ...
        if (validChanges.isBooked) {
            ...
            $scope.processChangeDoctorSchedule($scope.originalScheduleIds, $scope.selectedShifts,$scope.selectedDates, false)
        } else {
            if (convertOriginalDate !== $scope.selectedDates) {
                $scope.processChangeDoctorSchedule($scope.originalScheduleIds, $scope.selectedShifts,$scope.selectedDates, true)
            } else {
                $scope.processChangeDoctorSchedule($scope.originalScheduleIds, $scope.selectedShifts,$scope.selectedDates, false)
            }
        }
